...
const userSchema = new Schema({
  ...
  email: {type: String, required: true, index: true, unique:true, default: ""},
...
//List users contained in the email list
exports.listByEmail = async(email_list) => {

    try{
        var users = await User.find({ email: { $in: email_list } });
        return users || [];
    }
    catch{
        return [];
...
exports.patch = async(email, userData) => {
