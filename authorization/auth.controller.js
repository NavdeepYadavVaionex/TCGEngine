...
exports.Login = (req, res) => {
    try {
        ...
        let refreshId = req.login.email + jwtSecret;
        ...
        UserModel.patch(req.login.email, {refresh_key: refresh_key, proof_key: "", password_recovery_key: "", last_login_time: new Date()});
...
exports.ValidateToken = async(req, res, next) => {
    ...
    var token = req.jwt;
    var data = {email: token.email, login_time: new Date(token.iat * 1000), server_time: new Date() };
...
exports.ValidateProof = async(req, res) =>
{
    ...
    if(!email || typeof email != "string" || !proof || typeof proof != "string")
