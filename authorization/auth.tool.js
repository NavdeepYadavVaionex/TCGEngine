...
AuthTool.isRefreshValid = async(req, res, next) => {
    ...
    try {
        ...
        let hash = crypto.createHmac('sha512', req.jwt.refresh_key).update(req.jwt.email + config.jwt_secret).digest("base64");
        if (hash !== refresh_token)
            return res.status(403).send({error: 'Invalid refresh token'});
		
		//Validate refresh key in DB
        var user = await UserModel.getByEmail(req.jwt.email);
        if(!user)
...
AuthTool.isSameUserOr = (required_permission) => {
    return (req, res, next) => {
        ...
        let same_user = (req.jwt.email === email);
        if (email && same_user) {
            return next();
        } else {
            if (user_permission_level >= required_permission) {
