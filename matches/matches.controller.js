...
exports.addMatch = async(req, res) => {
    ...
    var player0 = await UserModel.getByEmail(players[0]);
    var player1 = await UserModel.getByEmail(players[1]);
    if(!player0 || !player1)
...
exports.completeMatch = async(req, res) => {
    ...
    var player0 = await UserModel.getByEmail(match.players[0]);
    var player1 = await UserModel.getByEmail(match.players[1]);
    if(!player0 || !player1)
        return res.status(404).send({error:"Can't find players"});
    ...
    if(match.ranked)
    {
        match.udata[0].reward = await MatchTool.GainMatchReward(player0, player1, player0.elo, player1.elo, winner);
        match.udata[1].reward = await MatchTool.GainMatchReward(player1, player0, player1.elo, player0.elo, winner);
...
exports.getLatest = async(req, res) => {
    ...
    var match = await MatchModel.getLast(req.params.email);
    if(!match)
        return res.status(404).send({error: "Match not found for user " + req.params.email});
